...
class CalculationsTest < MiniTest::Unit::TestCase
  def setup
    ...
    [@movie1, @movie2, @movie3, @book4, @book5, @book6, @book7, @book8, @movie9, @movie10].each { |obj| @user1.gem(obj) }
    [@book1, @book2, @book3, @movie4, @movie5, @movie6, @movie7, @movie8, @book9, @book10].each { |obj| @user1.disgem(obj) }

    # @user.similarity_with(@user2) should ==  0.25
    [@movie1, @movie2, @movie3, @book4, @book5, @book6].each { |obj| @user2.gem(obj) }
    [@book1, @book2, @book3].each { |obj| @user2.gem(obj) }
    ...
    [@movie1, @movie2, @movie3].each { |obj| @user4.gem(obj) }
    [@book1, @book2, @book3, @movie4, @movie5, @movie6].each { |obj| @user4.gem(obj) }
  ...
  def test_update_recommendations_ignores_rated_items
    ...
    movies = @user.gemd_movies + @user.disgemd_movies
    books  = @user.gemd_books  + @user.disgemd_books
